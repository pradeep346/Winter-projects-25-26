...
def train_maml(Xs, Ys, Xq, Yq, seed=SEED):
    ...
    for it in range(1, META_ITERS + 1):
        ...
        for idx in idxs:
            ...
            # FOMAML: copy gradients from adapted → original model
            with torch.no_grad():
...
def plot_losses(maml_losses, reptile_losses):
    ...
    ax.plot(smooth(maml_losses),    label="MAML",    alpha=0.85, linewidth=1.5)
    ax.plot(smooth(reptile_losses), label="Reptile", alpha=0.85, linewidth=1.5)
